...
## Core operating rule
Treat the ChatGPT Project as the **working memory and workbench**, but treat **GitHub** as the external source of truth for prompts, scripts, workflows, outputs, split-test files, and control files.

## Session start rule
For any meaningful ETF architecture, debugging, prompt, split-test, script, or workflow task, start by reading in this order:
...
## Production-versus-split rule
- `etf.txt` remains the protected production prompt.
- `prompts/as_is_split/ETF_RUNTIME_SPLIT.txt` is for split comparison runs only.
- `output_split_test/` is for comparison artifacts and must not silently replace production output.

## Quality rules
- Preserve determinism.
- Preserve the premium client-grade delivery standard.
- Preserve the ETF executive look & feel as the family reference design.
...
## Lean bootstrap rule
The default standing project upload should be:
- `control/PROJECT_BOOTSTRAP.md`

Do not default to uploading changing repo files unless the task truly requires it.
